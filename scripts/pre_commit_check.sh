...
if has_prefix "skills/" || has_prefix "tests/fixtures/retrieval_evaluation/" \
  || has_exact "scripts/generate_golden_queries.py" || has_exact "scripts/validate_eval_corpus.py"; then
  run_corpus=true
fi
...
if [[ "$run_skills" == true ]]; then
  ...
  if [[ ${#staged_skills[@]} -gt 0 ]]; then
    ...
    python3 scripts/validate_skill_trust.py --ci-gate "${staged_skills[@]}"
  fi
fi
