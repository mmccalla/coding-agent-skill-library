#!/usr/bin/env python3
"""Apply Phase 4: extract product-specific DataOps/MCP guidance to an overlay."""
...
SKILLS = ROOT / "skills"
OVERLAY = ROOT / "skills_docs" / "overlays" / "mas-dataops-mcp-overlay.md"
POINTER = (
    "## Optional overlay\n\n"
    "For product-specific DataOps/MCP examples, load "
    "`skills_docs/overlays/mas-dataops-mcp-overlay.md`."
)

REPLACEMENTS = {
    "skills/reliability-and-delivery/observability-and-telemetry/SKILL.md": (
        r"\n## Required metrics for MAS DataOps MCP\s*\n\n```text\n.*?\n```\n",
        "\n" + POINTER + "\n",
    ),
    "skills/reliability-and-delivery/sre-practice/SKILL.md": (
        r"\n## MAS DataOps MCP guidance\s*\n\n.*?(?=\n## Verification)",
        "\n" + POINTER + "\n",
    ),
    "skills/reliability-and-delivery/slo-error-budget-management/SKILL.md": (
        r"\n## MAS DataOps MCP guidance\s*\n\n.*?(?=\n## Verification)",
        "\n" + POINTER + "\n",
    ),
    "skills/reliability-and-delivery/dora-four-keys/SKILL.md": (
        r"\n## MAS DataOps MCP examples\s*\n\n.*?(?=\n## Verification)",
        "\n" + POINTER + "\n",
    ),
    "skills/reliability-and-delivery/incident-response-and-postmortems/SKILL.md": (
        r"\n## MAS DataOps MCP incident examples\s*\n\n.*?(?=\n## Verification)",
        "\n" + POINTER + "\n",
    ),
    "skills/reliability-and-delivery/release-engineering-and-progressive-delivery/SKILL.md": (
        r"\n## MAS DataOps MCP release guidance\s*\n\n.*?(?=\n## Verification)",
        "\n" + POINTER + "\n",
    ),
    "skills/reliability-and-delivery/toil-reduction-and-automation/SKILL.md": (
        r"\n## MAS DataOps MCP examples\s*\n\n.*?(?=\n## Verification)",
        "\n" + POINTER + "\n",
    ),
    "skills/user-experience/accessibility-wcag/SKILL.md": (
        r"\n## DataOps-specific guidance\s*\n\n.*?(?=\n## Related skills)",
        "\n" + POINTER + "\n",
    ),
    "skills/user-experience/ux-design-principles/SKILL.md": (
        r"\n## DataOps-specific guidance\s*\n\n.*?(?=\n## Verification)",
        "\n" + POINTER + "\n",
    ),
    "skills/user-experience/frontend-state-and-interaction-design/SKILL.md": (
        r"\n## DataOps-specific guidance\s*\n\n.*?(?=\n## Verification)",
        "\n" + POINTER + "\n",
    ),
    "skills/user-experience/agentic-ux-patterns/SKILL.md": (
        r"\n## DataOps-specific guidance\s*\n\n.*?(?=\n## Verification)",
        "\n" + POINTER + "\n",
    ),
    "skills/user-experience/user-research-and-usability-testing/SKILL.md": (
        r"\n## DataOps-specific scenarios\s*\n\n.*?(?=\n## Verification)",
        "\n" + POINTER + "\n",
    ),
}

OVERLAY_TEXT = """# MAS DataOps MCP Overlay
...
def main() -> int:
    ...
    changed = [OVERLAY]
    for relative_path, (pattern, replacement) in REPLACEMENTS.items():
        ...
        if count == 0 and POINTER not in updated:
            raise ValueError(f"Expected one replacement in {relative_path}, found {count}")
        if updated != text:
