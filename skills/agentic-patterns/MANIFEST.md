...
|---|---|---|
| using-agent-skills | Starting a session or choosing which local skill applies | The task is already unambiguous and a specific skill is obvious |
| interview-me | The user's request is underspecified and intent must be extracted | The goal, audience and constraints are already clear |
| idea-refine | A rough idea needs structured shaping before implementation | A formal spec already exists |
...
| doubt-driven-development | A high-stakes or unfamiliar decision should be challenged before coding | The path is obvious and low risk |
...
- Existing local skills already cover `prompt-chaining`, `routing`, `parallelisation`, `reflection-and-verification`, `tool-use-function-calling`, `planning-and-task-decomposition`, `multi-agent-collaboration`, `memory-management`, `learning-and-adaptation` and `mcp-server-design`.
